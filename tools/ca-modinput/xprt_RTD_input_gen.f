...
c          Reads 1943 - 2018 input file and modifies for the Rad 2018 - RTD year simulation.
c
...
c
      IF(irad.eq.1) THEN
        outfile1="input_XPRT-1_RTD"
      ELSE
        outfile1="input_XPRT-2_RTD"
      ENDIF
c
...
c
  200 IF(nyrend.lt.10000) THEN
      ...
      ENDIF
      IF(irad.eq.1) THEN
        WRITE(20,"(a26,a1,i4,a1,i"//frmt//",a13)") 
...
c
      IF(irad.eq.1) THEN
        WRITE(20,"(a23,a9)") 'Restart File, ../xprt-1',
     >    '/restart,'
      ELSEIF(irad.eq.2) THEN
        WRITE(20,"(a23,a9)") 'Restart File, ../xprt-2',
     >    '/restart,'
      ELSE
...
c
      ...
      DO ipt=1,nplttim
        ...
        IF(plottimes(ipt).eq.2018) iocyrfnd=1
        IF((plottimes(ipt).ge.2018).and.(inpl.eq.0)) inpl=ipt
      ENDDO
c
