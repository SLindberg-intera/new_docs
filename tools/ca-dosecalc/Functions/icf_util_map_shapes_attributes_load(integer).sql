-- FUNCTION: public.icf_util_map_shapes_attributes_load(text, integer)

-- DROP FUNCTION public.icf_util_map_shapes_attributes_load(text, integer);
...
begin
	select 'insert into public.map_shapes_attributes (shp_id, prp_key, prp_value) select * from ('||
		string_agg('select distinct shp_id, '''||column_name||''' prp_key, '||
			...
			' from public.map_shapes ms join public.'||table_name||' sm using (gid) where (mdl_id, shp_file_id) = ('||mdl_id||','||shp_file_id||')', ' union all ' order by column_name
	...
	where (s.mdl_id, s.shp_file_nm) = (_mdlid, _shapefile) and column_name not in ('gid','geom','geog');
...
ALTER FUNCTION public.icf_util_map_shapes_attributes_load(text, integer)
    OWNER TO ca;
