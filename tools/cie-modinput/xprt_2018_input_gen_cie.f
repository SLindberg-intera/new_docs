c       ************************ PROGRAM xprt_2018_input_gen_cie.f ****************************
c          Reads SS input file and modifies for the 1943-2018 transport simulation.
c
c          Command line variables:
c             1) Area - 200E or 200W
c             2) Buffer - Enter "buffer" if the model has a buffer; "nobuffer" if not
c             3) Path to the file containing the 200 West Area material properties
c             4) Path to the file containing the 200 East Area material properties
c             5) Path to the file containing the CIE (H-3, I-129, Sr-90 and Tc-99, U, Cr, NO3, CN) solute properties
c
...
c
      ...
      CHARACTER WMATTR*80,EMATTR*80,SOLTR*80
...
c
      READ(args(3),"(a80)") WMATTR
      READ(args(4),"(a80)") EMATTR
      READ(args(5),"(a80)") SOLTR
c
      WRITE(*,*)
c
c --- Set up input/output files based on area and buffer switch 
c
      infile1="../ss/input_cie_SS"
      infile2="../ret/cie_tr_boundary_card.dat"
      ...
      infile4="../trOCcards/cie_Output_Control.dat"
      infile5="../trsurfcards/cie_surface_flux.txt"
      infile6="../sources/cie-src.card"
      infile9=SOLTR
      IF(ibuff.eq.1) THEN
        outfile1="input_CIE_XPRT_2018_with_buffer"
      ELSE
        outfile1="input_CIE_XPRT_2018_no_buffer"
      ENDIF
c
...
c
c --- Read SS input file and revise for contaminant transport input
c
...
c
  800 WRITE(20,"(2a)") 'file, input.bot, Dirichlet Aqueous, outflow, ',
     ...
     >  '---------------------------'
      WRITE(20,"(a)")
c
c --- Replace SS output control with transient output control
c
...
c
  900 WRITE(20,"(a)")
      ...
      ENDIF
c
c --- Replace SS surface flux with transient surface flux
c
...
c
 1020 READ(16,"(a1024)",END=9981) line1
      ...
      READ(line1,*) numsrc
      WRITE(*,*) numsrc,' contaminant sources'
      REWIND(16)
c
      IF(ibuff.eq.1) THEN
        OPEN(17,FILE=infile7,STATUS='OLD'
