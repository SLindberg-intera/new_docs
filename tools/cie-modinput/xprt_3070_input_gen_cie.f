c       ************************ PROGRAM xprt_3070_input_gen_cie.f ****************************
c          Reads 1943 - 2018 input file and modifies for the 2018 (or RTD year) - 3070 simulation.
c
...
c
c --- Read 2018 input file and revise for 3070 input
c
...
c
  100 READ(11,"(a1024)",END=9992) line1
      ...
      GOTO 100
c
  200 IF(nyrstrt.eq.2018) THEN
          WRITE(20,"(a25,a1,i4,a1,i4,a2)") 
     >      'CIE Transport Simulation ',
     >      '(',nyrstrt,'-',nyrend,'),'
      ELSE
          WRITE(20,"(a25,a1,i4,a14,i4,a2)") 
     >      'CIE Transport Simulation ',
...
c
     ...
     >  2070,',year,1.0E-08,year,0.1,year,1.25,16,1.0E-6,'
      WRITE(20,"(i4,a6,i4,a43)") 2070,',year,',
     >  3070,',year,1.0E-08,year,1.0,year,1.25,16,1.0E-6,'
c
      WRITE(20,"(a)") '1000000,'
      WRITE(20,"(a)") '0,'
      WRITE(20,"(2a)") '#---------------------------------------',
...
c
  800 llen=len(trim(line1))
      ...
      DO ipt=1,nplttim
        ...
        IF(ABS(plottimes(ipt)-nyrstrt).lt.1.0E-10) iocyrfnd=1
        IF(((plottimes(ipt)-nyrstrt).gt.-1.0E-10).and.(inpl.eq.0))
     >    inpl=ipt
      ENDDO
c
...
c
      IF(inpl.eq.0) THEN
        WRITE(*,*) ' STOP: No times gt ',nyrstrt
        GOTO 9999
      ELSE
        DO iocyr=1,inpl-1
