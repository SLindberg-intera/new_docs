...
shp="/opt/ICF/Prod/MFGRID/v8.3/data/grid_274_geo.shp"
#Stomp input file with grid
stomp_input=../ss/input_SS
#model name
model=mpond
#output file
output=./rad1_surface_flux.txt
#csv file
csv="./rad1_"$model"_grid_conversion.csv"
#list of COPCs
...
py=python3.6
$py $tool_runner --logfile $runner_log --logfilemode 'w' "$py" "$tool -c $copc -shp $shp -i $stomp_input -s $model -o $output -csv $csv"