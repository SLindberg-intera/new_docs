...
where :
     ...
     -f  the name of the shapefile to load
...
if [[ $fcnt == 1 ]]; then
  ...
  srid=$(psql -d "$dbase" -qtA -c "select srid from spatial_ref_sys where esri_text like '"$prj"%';")
fi
#echo "$srid"
...
shp2pgsql -G -c "$fpath"/"$fname" public."$tname"_geog | psql -q -d "$dbase"
...
sfid=-1
sfid=$(psql -d "$dbase" -qtA -c "insert into public.shapefiles (shp_file_nm, shp_file_url, mdl_id) values ('"$fname"', '"$fpath"/"$fname"', "$mdlid") returning shp_file_id;")
#echo "$sfid"
if [[ $sfid == -1 ]]; then
